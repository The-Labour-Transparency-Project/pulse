...
function serializeValue(item: SurveyItem, value: Exclude<Answers[string], undefined>, survey: SurveyDefinition, createId: () => string, rowId?: string): SerializedAnswer {
    const answer: SerializedAnswer = {
        id: createId(),
        itemId: item.id, ...(rowId ? {rowId} : {}),
        value,
        presented: true
    }
...
export function serializeSurveyResponse(
        ...
        for (const optionId of selected) {
            ...
            if (detail !== undefined && detail !== '') {
                serialized.push({
                    ...
                    optionSemantics: optionSemantics(item, [optionId], survey),
                    presented: true,
                })
            }
        }
    ...
    return {
        ...
        waveId: options.waveId ?? survey.versioning?.waveId ?? 'pulse-2026',
