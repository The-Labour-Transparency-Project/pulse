...
describe("survey drafts", () => {
    ...
    it("uses a survey and version scoped position key", () => {
        expect(positionStorageKey(survey)).toBe(`pulse-respondent-position:pulse-2026:${survey.id}:${survey.version}`);
    });

    it("stores and validates non-question destinations by survey version", () => {
        expect(destinationStorageKey(survey)).toBe(`pulse-respondent-destination:pulse-2026:${survey.id}:${survey.version}`);
    ...
    it("uses a survey and version scoped storage key", () => {
        expect(draftStorageKey(survey)).toBe(`pulse-respondent-draft:pulse-2026:${survey.id}:${survey.version}`);
    });
    ...
    it("rejects drafts from another instrument version", () => {
        expect(parseSurveyDraft({waveId: "pulse-2026", surveyId: survey.id, surveyVersion: "old", answers: {}}, survey)).toBeNull();
    });
