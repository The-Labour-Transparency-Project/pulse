...
import type { Answers, AnswerValue, DetailAnswers, SurveyItem, SurveyOption } from '../domain/survey'

const props = defineProps<{ item: SurveyItem; answers: Answers; detailAnswers: DetailAnswers; options: SurveyOption[]; rowOptions: Record<string, SurveyOption[]> }>()
const answer = computed({
  get: () => props.answers[props.item.id],
  set: (value) => { props.answers[props.item.id] = value },
})
