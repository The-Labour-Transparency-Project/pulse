...
export function useResizablePanels() {
    ...
    const tipsWidths = useLocalStorage<Record<PanelBreakpoint, number>>("pulse-respondent-tips-panel-widths", {
        xs: 420, sm: 500, md: 500, lg: 360, xl: 420,
    });
    ...
    const leftWidth = computed({
        get: () => (breakpoint.value === "xs" || breakpoint.value === "sm")
            ? panelDefaultsForBreakpoint(breakpoint.value).left
            : leftWidths.value[breakpoint.value] ?? panelDefaultsForBreakpoint(breakpoint.value).left,
        set: (value: number) => {
            leftWidths.value[breakpoint.value] = value;
        },
    });
    const rightWidth = computed({
        ...
        set: (value: number) => {
            rightWidths.value[breakpoint.value] = value;
        },
    });
    const tipsWidth = computed({
        get: () => (breakpoint.value === "xs" || breakpoint.value === "sm")
            ? panelDefaultsForBreakpoint(breakpoint.value).tips
            : tipsWidths.value[breakpoint.value] ?? panelDefaultsForBreakpoint(breakpoint.value).tips,
        set: (value: number) => {
            tipsWidths.value[breakpoint.value] = value;
        },
    });
